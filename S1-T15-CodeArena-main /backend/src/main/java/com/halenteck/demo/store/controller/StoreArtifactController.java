    ...
    public ResponseEntity<?> upload(@RequestParam("file") MultipartFile file,
        ...
        try {
            ...
            for (StoreArtifactEntity oldArtifact : existingVersions) {
                ...
                // Eğer storage_path null ise set et (eski kayıtlar için)
                if (oldArtifact.getStoragePath() == null) {
                    oldArtifact.setStoragePath("db://");
                }
                // Eğer storage_url null ise set et (eski kayıtlar için)
            ...
            // storage_path set et (Database constraint için gerekli, ama data DB'de saklanıyor)
            artifact.setStoragePath("db://");
            
            // storage_url set et (Database constraint için gerekli)
            // ID henüz yok, bu yüzden geçici bir değer koyuyoruz, save sonrası güncellenecek
            ...
            // 8. storage_url'i gerçek ID ile güncelle
